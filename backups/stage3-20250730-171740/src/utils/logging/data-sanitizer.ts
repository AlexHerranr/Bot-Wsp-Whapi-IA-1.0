 ...
 */
function sanitizeValue(value: any, config: SanitizationConfig, visited: WeakSet<object> = new WeakSet()): any {
    ...
    if (Array.isArray(value)) {
        return value.map(item => sanitizeValue(item, config, visited));
    }
    
    if (typeof value === 'object') {
        return sanitizeObject(value, config, visited);
    }
 ...
 * 🏗️ SANITIZAR OBJETO
 */
function sanitizeObject(obj: any, config: SanitizationConfig, visited: WeakSet<object> = new WeakSet()): any {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        // Casos especiales
        if (config.maskPhoneNumbers && (lowerKey.includes('phone') || lowerKey.includes('telefono') || lowerKey.includes('celular'))) {
        ...
        if (config.maskEmails && (lowerKey.includes('email') || lowerKey.includes('correo'))) {
        ...
        sanitized[key] = sanitizeValue(value, config, visited);
    }
