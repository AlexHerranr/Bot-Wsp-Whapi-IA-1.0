...
import { TempFunctionRegistry } from '../functions/registry/temp-function-registry';
...
export class CoreBot {
    ...
    private functionRegistry: TempFunctionRegistry;
    ...
    constructor(config: AppConfig) {
        ...
        this.functionRegistry = new TempFunctionRegistry();
        
        // Register hotel plugin functions
        this.hotelPlugin.register(this.functionRegistry);
    ...
    private async processBufferCallback(userId: string, combinedText: string, chatId: string, userName: string): Promise<void> {
        ...
        try {
            ...
            // 6. Generate hotel-specific context if needed
            const profile = { name: userName };
            const chatInfo = { name: userName, labels: [] };
            const hotelContext = await this.hotelPlugin.context.getRelevantContext(userId, profile, chatInfo);
            
            // 7. Validate response using hotel plugin
            const validationResult = this.hotelPlugin.validation.validateAndCorrectResponse(response, []);
            ...
            await this.whatsappService.sendWhatsAppMessage(chatId, finalResponse, userState, isQuote);
            this.mediaManager.addBotSentMessage(`msg_${Date.now()}`);

            this.terminalLog.response(userName, finalResponse, 1000); // Simular 1s de duración
