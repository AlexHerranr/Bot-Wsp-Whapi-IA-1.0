...
const prompt_variables_service_1 = require("./prompt-variables.service");
class OpenAIResponsesService {
    constructor(config, terminalLog, cacheManager, functionRegistry, whatsappService, databaseService, userManager, mediaService) {
        ...
        this.responseService = new response_service_1.ResponseService({
            apiKey: config.apiKey,
            model: config.model || 'gpt-5',
            maxOutputTokens: config.maxOutputTokens || 4096,
            temperature: config.temperature || 0.7
        }, functionRegistry);
        this.conversationManager = new conversation_manager_1.ConversationManager(databaseService);
        this.promptVariablesService = new prompt_variables_service_1.PromptVariablesService(databaseService);
        ...
        if (process.env.OPENAI_PROMPT_ID && process.env.USE_PROMPT_ID !== 'false') {
            ...
            if (process.env.OPENAI_PROMPT_VERSION) {
                this.systemInstructions.version = process.env.OPENAI_PROMPT_VERSION;
            }
            this.usePromptId = true;
            (0, logging_1.logInfo)('PROMPT_CONFIG', 'Usando prompt ID del dashboard', {
            ...
            this.usePromptId = false;
            (0, logging_1.logInfo)('PROMPT_CONFIG', 'Usando instrucciones inline');
        }
    }
...
Tienes acceso a funciones para consultar disponibilidad, crear reservas y obtener información actualizada.`;
    ...
    async processMessage(userId, message, chatId, userName, existingThreadId, existingTokenCount, imageMessage, duringRunMsgId, assistantId) {
        const startTime = Date.now();
        // Control de concurrencia
        if (OpenAIResponsesService.activeOpenAICalls >= OpenAIResponsesService.MAX_CONCURRENT_CALLS) {
            (0, logging_1.logWarning)('OPENAI_RATE_LIMIT', 'Límite de llamadas concurrentes alcanzado', {
                userId,
        ...
        try {
            ...
            try {
                ...
                (0, logging_1.logWarning)('GET_USER_LABELS_ERROR', 'Error obteniendo etiquetas del usuario', { userId });
                userLabels = [];
            }
            // NO ENVIAR VARIABLES - El prompt no las necesita
            let promptVariables = undefined;
            ...
            */
            ...
            (0, logging_1.logDebug)('FUNCTIONS_DECISION', 'Decisión sobre funciones', {
                usingPromptId,
            ...
            (0, integrations_1.logOpenAIPromptSent)(userId, context.previousResponseId || 'new', message, message.length);
            // Llamar a Responses API
            const result = await this.responseService.createResponse(this.systemInstructions, enrichedMessage, // Usar mensaje enriquecido
            conversationContext, functions, imageMessage // Pasar imagen directamente a GPT-5
            );
            if (!result.success) {
            ...
            if (result.functionCalls && result.functionCalls.length > 0) {
                const functionResults = await this.responseService.executeFunctionCalls(result.functionCalls, conversationContext);
                ...
                const followUpResult = await this.responseService.createResponse(this.systemInstructions, functionResultsMessage, {
                    ...conversationContext,
            ...
            await this.conversationManager.updateConversation(userId, chatId, result.responseId, result.usage?.totalTokens || 0);
            // Guardar respuesta del asistente
            await this.conversationManager.addMessage(userId, chatId, 'assistant', finalResponse, result.responseId);
            // IMPORTANTE: Enviar respuesta a WhatsApp
            if (this.whatsappService && finalResponse) {
                await this.whatsappService.sendWhatsAppMessage(chatId, finalResponse, { lastInputVoice: false }, // TODO: Obtener del contexto si fue audio
                false // isQuoteOrPrice
                );
            }
            // Log de uso de tokens
            if (result.usage) {
                (0, integrations_1.logTokenUsage)(userId, context.previousResponseId || 'no-conversation', result.usage.inputTokens, result.usage.outputTokens, 'gpt-5');
            }
            // Log de flujo completo
            (0, integrations_1.logMessageFlowComplete)(userId, message.length, finalResponse.length, Date.now() - startTime, result.usage?.totalTokens || 0, result.functionCalls?.length || 0);
            return {
                success: true,
            ...
            (0, logging_1.logError)('PROCESS_MESSAGE_ERROR', 'Error procesando mensaje', {
                userId,
    ...
    getFunctionsForRequest() {
        ...
        if (!this.functionRegistry) {
            (0, logging_1.logDebug)('FUNCTIONS_REGISTRY', 'FunctionRegistry no configurado, no enviar tools');
            return [];
        }
        const functions = this.functionRegistry.getAllFunctions();
        // Si no hay funciones definidas, devolver array vacío
        if (!functions || functions.length === 0) {
            (0, logging_1.logDebug)('FUNCTIONS_EMPTY', 'No hay funciones definidas en el registry');
            return [];
        }
        (0, logging_1.logDebug)('FUNCTIONS_LOADED', `Cargando ${functions.length} funciones para el request`);
        // Log detallado de las funciones
        (0, logging_1.logDebug)('FUNCTIONS_RAW', 'Funciones del registry', {
        ...
        const validFunctions = functions.filter(fn => {
            if (!fn.name) {
                (0, logging_1.logWarning)('FUNCTION_MISSING_NAME', 'Función sin nombre, ignorando', {
        ...
        (0, logging_1.logDebug)('FUNCTIONS_VALIDATED', `${validFunctions.length} funciones válidas para el request`, {
    ...
    updateSystemInstructions(instructions) {
        ...
        (0, logging_1.logInfo)('SYSTEM_INSTRUCTIONS_UPDATED', 'Instrucciones del sistema actualizadas', {
    ...
    async processWithOpenAI(userId, combinedText, chatId, userName) {
        await this.processMessage(userId, combinedText, chatId, userName);
    }
}
exports.OpenAIResponsesService = OpenAIResponsesService;
OpenAIResponsesService.activeOpenAICalls = 0;
OpenAIResponsesService.MAX_CONCURRENT_CALLS = 75;
