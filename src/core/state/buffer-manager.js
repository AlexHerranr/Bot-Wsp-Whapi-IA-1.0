...
(0, logging_1.logInfo)('BUFFER_CONFIG', 'Configuración de buffer timing', {
    BUFFER_DELAY_MS: constants_1.BUFFER_DELAY_MS,
    MAX_BUFFER_MESSAGES: constants_1.MAX_BUFFER_MESSAGES,
    envValue: process.env.BUFFER_DELAY_MS || 'not_set',
    actualValue: constants_1.BUFFER_DELAY_MS
}, 'buffer-manager.ts');
class BufferManager {
    constructor(processCallback, getUserState) {
        this.processCallback = processCallback;
        this.getUserState = getUserState;
        this.buffers = new Map();
        this.activeRuns = new Map(); // Flag simple para evitar concurrent runs per user/thread
        this.activeRunTimestamps = new Map(); // Timestamps para timeout de seguridad
    }
    addMessage(userId, messageText, chatId, userName, quotedMessageId, currentMessageId) {
        ...
        if (!buffer) {
            ...
            (0, logging_1.logInfo)('BUFFER_CREATED', 'Buffer creado para mensaje', { userId, userName, reason: 'new_message' }, 'buffer-manager.ts');
        }
        else {
            if (userName && userName !== 'Usuario')
                buffer.userName = userName;
            if (chatId && !buffer.chatId)
                buffer.chatId = chatId;
            // SIMPLIFICADO: quotedMessageId eliminado - solo duringRunMsgId
        }
        if (buffer.messages.length >= constants_1.MAX_BUFFER_MESSAGES) {
            (0, logging_1.logWarning)('BUFFER_LIMIT_REACHED', 'Límite alcanzado, procesando inmediatamente', { userId, userName, messageCount: buffer.messages.length }, 'buffer-manager.ts');
            this.processBuffer(userId);
            return;
        }
        if (buffer.messages.length > 0 && buffer.messages[buffer.messages.length - 1] === messageText) {
            (0, logging_1.logInfo)('BUFFER_DUPLICATE_SKIP', 'Mensaje duplicado omitido', { userId, userName, messageText: messageText.substring(0, 50) });
            return;
        }
        ...
        if (this.activeRuns.get(userId)) {
            ...
            (0, logging_1.logInfo)('CITACION_AUTO_MARK', 'Mensaje marcado para citación auto durante run', {
                userId,
                userName,
        ...
        if (buffer.messages.length > 0) {
            (0, logging_1.logDebug)('BUFFER_STATE', 'Estado actual del buffer al añadir mensaje', {
                userId,
                userName,
        ...
        (0, logging_1.logDebug)('BUFFER_STATE_ADD', 'Mensaje agregado, estado actual buffer', {
            userId,
        ...
        (0, logging_1.logDebug)('MESSAGE_ORDER_TRACKING', 'Orden de mensajes en buffer', {
            userId,
    ...
    addImageMessage(userId, imageMessage, chatId, userName) {
        let buffer = this.buffers.get(userId);
        if (!buffer) {
            buffer = { messages: [], chatId, userName, lastActivity: Date.now(), timer: null, pendingImage: null };
            this.buffers.set(userId, buffer);
            (0, logging_1.logInfo)('BUFFER_CREATED', 'Buffer creado para imagen', { userId, userName, reason: 'image_message' }, 'buffer-manager.ts');
        }
        else {
            if (userName && userName !== 'Usuario')
                buffer.userName = userName;
        ...
        (0, logging_1.logInfo)('BUFFER_IMAGE_ADDED', 'Imagen agregada al buffer', {
            userId,
            userName: buffer.userName || 'Usuario',
    ...
    onTypingOrRecording(userId) {
        ...
        if (buffer.messages.length > 0 || buffer.pendingImage) {
            ...
            (0, logging_1.logDebug)('BUFFER_EXTEND_ACTIVITY', 'Timer extendido por actividad', {
                userId,
                userName: buffer.userName || 'Usuario',
                messageCount: buffer.messages.length,
    ...
    setOrExtendTimer(userId, reason = 'message') {
        ...
        if (isVoiceContext) {
            ...
            delay = isOperations ? 1000 : constants_1.BUFFER_DELAY_MS; // Operations: 1s, Main: 5s para texto
        }
        // Cancelar timer anterior si existe (last event wins)
        if (buffer.timer) {
            clearTimeout(buffer.timer);
            (0, logging_1.logInfo)('BUFFER_TIMER_CANCEL', 'Timer anterior cancelado', {
                userId,
                userName: buffer.userName || 'Usuario',
                reason: 'new_event',
                previousDelay: delay
        ...
        const textCount = buffer.messages.filter(msg => !msg.includes('(Nota de Voz Transcrita por Whisper)')).length;
        const voiceCount = buffer.messages.filter(msg => msg.includes('(Nota de Voz Transcrita por Whisper)')).length;
        (0, logging_1.logInfo)('BUFFER_STATE_WAIT', 'Buffer esperando agrupación', {
            userId,
            userName: buffer.userName || 'Usuario',
            currentMsgs: buffer.messages.length,
            types: `Text:${textCount} Voz:${voiceCount}`,
    ...
    async processBuffer(userId) {
        ...
        if (!buffer || (buffer.messages.length === 0 && !buffer.pendingImage)) {
            (0, logging_1.logInfo)('BUFFER_EMPTY_SKIP', 'Buffer vacío, limpiando', {
                userId,
        ...
        */
        ...
        (0, logging_1.logDebug)('BUFFER_RESPECTING_TIMER', 'Respetando timer de buffer para mantener orden', {
            userId,
            messageCount: buffer.messages.length,
        ...
        if (this.activeRuns.get(userId)) {
            ...
            if (runDuration > 60000) { // 60 segundos
                (0, logging_1.logWarning)('BUFFER_FORCE_RELEASE_TIMEOUT', 'Liberando run atascado por timeout de seguridad', {
                    userId,
                ...
                (0, logging_1.logWarning)('BUFFER_DELAYED_ACTIVE_RUN', 'Retrasando por run activo', {
                    userId,
                    userName: buffer.userName,
                    pendingMessages: buffer.messages.length,
                    runDuration
        ...
        if (buffer.isVoice && (Date.now() - buffer.lastActivity > 3000)) {
            (0, logging_1.logInfo)('BUFFER_VOICE_FAST_PATH', 'Procesando buffer de voz tras 3s de inactividad', {
                userId,
                userName: buffer.userName,
                messageCount: buffer.messages.length,
            ...
            if (this.getUserState) {
                const userState = this.getUserState(userId);
                if (userState?.isTyping || userState?.isRecording) {
                    (0, logging_1.logInfo)('BUFFER_DELAYED_USER_ACTIVE', 'Usuario activo, extendiendo buffer', {
                        userId,
                        userName: buffer.userName,
        ...
        if (isDuringRunPending) {
            (0, logging_1.logInfo)('BUFFER_PENDING_DURING_RUN', 'Procesando con citación auto', {
                userId,
                userName,
                duringRunMsgId: buffer.duringRunMsgId,
                messageCount: buffer.messages.length
        ...
        (0, logging_1.logSuccess)('BUFFER_GROUPED', 'Procesando idea completa agrupada', {
            userId,
            userName,
        ...
        try {
            ...
            (0, logging_1.logWarning)('BUFFER_PROCESS_ERROR', 'Error procesando buffer', {
                userId,
                userName,
            ...
            if (currentBuffer) {
                ...
                (0, logging_1.logInfo)('FLAG_RESET', 'duringRunMsgId reseteado post-procesamiento', { userId });
            }
            ...
            if (currentBuffer && currentBuffer.messages.length > 0) {
                ...
                (0, logging_1.logInfo)('BUFFER_PENDING_AFTER_RUN', 'Mensajes pendientes detectados después del run', {
                    userId,
    ...
    cleanup(maxAge = 15 * 60 * 1000) {
        ...
        for (const [userId, buffer] of this.buffers.entries()) {
            if (now - buffer.lastActivity > maxAge) {
                (0, logging_1.logInfo)('BUFFER_CLEANUP_OLD', 'Limpiando buffer inactivo', {
                    userId,
                    userName: buffer.userName,
        ...
        if (cleaned > 0)
            (0, logging_1.logInfo)('BUFFER_CLEANUP_COMPLETE', 'Limpieza completada', { cleaned, remaining: this.buffers.size }, 'buffer-manager.ts');
        return cleaned;
    }
    // Métodos legacy para compatibilidad
    setIntelligentTimer(userId, triggerType) {
        ...
        (0, logging_1.logDebug)('BUFFER_TIMEOUT_DYNAMIC', 'Timeout configurado por processor', {
            userId,
            triggerType,
            isOperations,
            timeoutMs,
        ...
        if (triggerType === 'typing' || triggerType === 'recording') {
            ...
            if (!buffer) {
                buffer = { messages: [], chatId: '', userName: '', lastActivity: Date.now(), timer: null };
                this.buffers.set(userId, buffer);
                (0, logging_1.logInfo)('BUFFER_CREATED', 'Buffer creado por timer inteligente', { userId, triggerType }, 'buffer-manager.ts');
            }
            // Para voice, log especial indicando espera de transcripción
            if (triggerType === 'voice') {
                (0, logging_1.logInfo)('BUFFER_VOICE_WAIT', 'Esperando transcripción de voz', {
                    userId,
                    userName: buffer.userName,
                    delay: constants_1.BUFFER_DELAY_MS
                }, 'buffer-manager.ts');
                ...
                if (buffer.messages.length > 0 || buffer.pendingImage) {
                    ...
                    (0, logging_1.logInfo)('BUFFER_TIMER_SKIP', 'Timer no establecido en setIntelligentTimer', {
                        userId,
                        triggerType,
                        reason: 'empty_buffer'
                    }, 'buffer-manager.ts');
    ...
    markRecentError(userId) {
        const buffer = this.buffers.get(userId);
        if (buffer) {
            (0, logging_1.logInfo)('BUFFER_ERROR_CLEANUP', 'Limpiando buffer por error', {
                userId,
                userName: buffer.userName,
                messageCount: buffer.messages.length
            }, 'buffer-manager.ts');
