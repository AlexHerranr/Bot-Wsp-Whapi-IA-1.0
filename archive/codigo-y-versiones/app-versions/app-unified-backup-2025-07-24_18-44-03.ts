...
// 🔧 RESTAURADO: Sistema terminalLog limpio del código antiguo
const terminalLog = {
    message: (user: string, text: string) => console.log(`👤 ${user}: "${text.substring(0, 60)}${text.length > 60 ? '...' : ''}"`),
    typing: (user: string) => console.log(`✍️ ${user} está escribiendo...`),
    processing: (user: string) => {}, // Vacío para evitar ruido
    response: (user: string, text: string, duration: number, messageCount?: number) => {
        const msgInfo = messageCount ? `${messageCount} msgs → ` : '';
        console.log(`🤖 OpenAI → ${msgInfo}${user}: "${text.substring(0, 60)}${text.length > 60 ? '...' : ''}" (${duration}s)`);
    },
    ...
    startup: () => {
        ...
        console.log(`✅ Servidor HTTP iniciado en ${appConfig?.host || 'localhost'}:${appConfig?.port || 3008}`);
        console.log(`🔗 Webhook URL: ${appConfig?.webhookUrl || 'configurando...'}`);
    },
    newConversation: (user: string) => console.log(`\n📨 Nueva conversación con ${user}`),
    image: (user: string) => console.log(`📷 ${user}: [Imagen recibida]`),
    voice: (user: string) => console.log(`🎤 ${user}: [Nota de voz recibida]`)
};
...
// Eliminado: Control de procesamiento activo que causaba bloqueos
...
// Rate limiting removido - logs directos como código antiguo
...
const main = async () => {
    try {
        ...
        server.listen(appConfig.port, appConfig.host, () => {
            // 🔧 NUEVO: Usar terminalLog.startup() para mostrar logs limpios
            terminalLog.startup();
    ...
    app.post('/hook', async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (messages && Array.isArray(messages) && messages.length > 0) {
                // Log eliminado - se usa terminalLog.message() en cada mensaje individual
...
async function processGlobalBuffer(userId: string): Promise<void> {
    ...
    // Agregar log de procesamiento como en código antiguo
    terminalLog.processing(buffer.userName);
...
function updateTypingStatus(userId: string, isTyping: boolean): void {
    ...
    // 🔧 MEJORADO: Timer unificado de 5 segundos (del código antiguo)
    if (buffer.timer) {
    ...
    // 🔧 MEJORADO: Log de terminal más informativo (del código antiguo)
...
function addToGlobalBuffer(userId: string, messageText: string, chatId: string, userName: string, isVoice: boolean = false): void {
    ...
    // 🔧 OPTIMIZADO: Usar delay más corto para media (2s) vs texto normal (5s) - EXACTAMENTE como código antiguo
    ...
    // 🔧 RESTAURADO: Log limpio como el código antiguo
    terminalLog.message(userName, messageText);
        ...
        if (chunks.length === 1) {
            ...
            const response = await fetch(`${appConfig.secrets.WHAPI_API_URL}/messages/text`, {
                ...
                body: JSON.stringify({
                    to: chatId,
                    body: message,
                    typing_time: message.includes('🔄') || message.includes('📊') ? 5 : 3 // Como código antiguo
                })
            });
            ...
            // Log manejado por terminalLog.response más adelante
            return true;
        } else {
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                const response = await fetch(`${appConfig.secrets.WHAPI_API_URL}/messages/text`, {
                    ...
                    body: JSON.stringify({
                        to: chatId,
                        body: chunk,
                        typing_time: i === 0 ? 3 : 2 // 3s primer mensaje, 2s siguientes - EXACTAMENTE como código antiguo
                    })
                });
...
function setupWebhooks() {
    ...
    processCombinedMessage = async function(userId: string, combinedText: string, chatId: string, userName: string, messageCount: number): Promise<void> {
        ...
        const processFunction = async () => {
            ...
            if (threadRecord) {
                try {
                    ...
                    if (activeRuns.length > 0) {
                        ...
                        // 🔧 ARREGLO: Eliminar salto por run activo - procesar siempre como el código antiguo
                        // El código antiguo no verificaba runs activos y siempre procesaba después del buffer
                        // Mantener solo el manejo de requires_action pero continuar con el procesamiento
                    }
                } catch (error) {
            ...
            // 🔧 MEJORADO: Log de inicio de procesamiento (del código antiguo)
            console.log(`🤖 [BOT] ${messageCount} msgs → OpenAI`);
            
            // Todo mensaje relevante va directo a OpenAI
            const startTime = Date.now();
            const response = await processWithOpenAI(combinedText, userId, chatId, userName, undefined, messageCount);
            const aiDuration = ((Date.now() - startTime) / 1000).toFixed(1);
            
            // 🔧 MEJORADO: Log de respuesta más informativo (del código antiguo)
            if (response && response.trim()) {
                ...
                if (willSplit) {
                    ...
                    terminalLog.response(userName, `${paragraphCount} párrafos`, aiDuration, messageCount);
                } else {
                    const preview = response.length > 50 ? response.substring(0, 50) + '...' : response;
                    terminalLog.response(userName, preview, aiDuration, messageCount);
                }
            } else {
                console.log(`❌ [BOT] Sin respuesta (${aiDuration}s)`);
        ...
        // 🔧 ETAPA 2.2: Sin retries para flujo directo como código antiguo
        let retryCount = 0;
        const maxRetries = 0; // Flujo directo como código antiguo
    ...
    async function subscribeToPresence(userId: string): Promise<void> {
        ...
        try {
            // 🔧 NUEVO: Agregar delay de 1s como código antiguo para evitar race conditions
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Verificar de nuevo después del delay (otro hilo pudo haber suscrito)
            if (subscribedPresences.has(userId)) {
                return; // Ya suscrito por otro hilo
            }
    ...
    async function processUserMessages(userId: string) {
        ...
        const processFunction = async () => {
            ...
            const startTime = Date.now();
            const response = await processWithOpenAI(combinedMessage, userId, buffer.chatId, buffer.userName, requestId, buffer.messages.length);
            const aiDuration = ((Date.now() - startTime) / 1000).toFixed(1);
    ...
    const processWithOpenAI = async (userMsg: string, userJid: string, chatId: string = null, userName: string = null, requestId?: string, messageCount?: number): Promise<string> => {
        ...
        try {
            ...
            // Log eliminado - se usa terminalLog.response() al final con duración
            ...
            logInfo('HISTORY_STRATEGY', 'OpenAI decide contexto bajo demanda - sin inyección automática', {
             ...
             while (addAttempts < maxAddAttempts) {
                 try {
                    ...
                    await openaiClient.beta.threads.messages.create(threadId, {
                     ...
                     // Log eliminado - se usa solo logs técnicos sin terminal
             ...
             // Crear y ejecutar run (log eliminado)
             let run = await openaiClient.beta.threads.runs.create(threadId, {
                 assistant_id: secrets.ASSISTANT_ID
             });
             
             // Log eliminado - se usa terminalLog.response() al completar
            ...
            if (run.status === 'completed') {
                ...
                // Log eliminado - se usa terminalLog.response() al final
                ...
                // Log removido - ya se hace en terminalLog.response() para evitar duplicados
                ...
                // Log de respuesta como en código antiguo
                const durationSeconds = ((Date.now() - startTime) / 1000);
                terminalLog.response(userName || shortUserId, responseText, durationSeconds, messageCount);
            ...
            if (error?.code === 'context_length_exceeded' || 
                ...
                try {
                    ...
                    return await processWithOpenAI(userMsg, userJid, chatId, userName, requestId, messageCount);
...
async function initializeBot() {
    ...
    setTimeout(async () => {
        try {
            // Log removido - ya se hace dentro de recoverOrphanedRuns()
    ...
    try {
        ...
        if (presences && event?.type === 'presences' && event?.event === 'post') {
            ...
            presences.forEach((presence: { contact_id: string, status: string }) => {
                ...
                if (status === 'typing' || status === 'recording') {
                    ...
                    // Log eliminado - se usa terminalLog.typing() en updateTypingStatus()
        ...
        for (const message of messages) {
            try {
                    ...
                    buffer.timer = setTimeout(async () => {
                        ...
                        if (finalBuffer && finalBuffer.messages.length > 0) {
                            ...
                            try {
                                ...
                                // Log eliminado - solo logs técnicos para mensajes manuales del agente
                ...
                // Log eliminado - se usa terminalLog.message() para cada tipo específico
